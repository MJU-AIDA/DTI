...
def process_files_ddi(files, triple_file, saved_relation2id=None, keeptrainone = False):
    ...
    for i in range(rel):
        idx = np.argwhere(triplets['train'][:, 2] == i)
        adj_list.append(csc_matrix((np.ones(len(idx), dtype=np.uint8), (triplets['train'][:, 0][idx].squeeze(1), \
        triplets['train'][:, 1][idx].squeeze(1))), shape=(len(entity2id), len(entity2id))))
    for i in range(rel, len(relation2id)):
        idx = np.argwhere(kg_triple[:, 2] == i-rel)
        #print("++++++++++++", idx)
        #print(len(entity2id))
        #print(len(idx), idx)
        #print(kg_triple.shape)
        #print(np.ones(len(idx)).shape)
        #print(kg_triple[:, 0][idx].squeeze(1).shape)
        #print(kg_triple[:, 1][idx].squeeze(1).shape)
        #print(np.max(kg_triple[:, 0][idx].squeeze(1)))
        #print(np.max(kg_triple[:, 1][idx].squeeze(1)))
        #print(ma)
        csc_matrix((np.ones(len(idx), dtype=np.uint8), (kg_triple[:, 0][idx].squeeze(1), kg_triple[:, 1][idx].squeeze(1))), shape=(len(entity2id), len(entity2id)))
        adj_list.append(csc_matrix((np.ones(len(idx), dtype=np.uint8), (kg_triple[:, 0][idx].squeeze(1), \
        kg_triple[:, 1][idx].squeeze(1))), shape=(len(entity2id), len(entity2id))))
        #print(adj_list)
    #print(adj_list)
    #assert 0
    return adj_list, triplets, entity2id, relation2id, id2entity, id2relation, rel
